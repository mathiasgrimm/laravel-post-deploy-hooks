...
it('passes repeatable key value options to the target as named constructor arguments', function () {
    $status = app(Kernel::class)->handle(new ArgvInput([
        'artisan', 'post-deploy-hooks', '--deploy-version=release-b', '--job='.JobWithArguments::class,
...
it('retains arguments through retries for the failure callback', function () {
    ...
    config(['post-deploy-hooks.job.failure_handler' => FailureHandler::class, 'post-deploy-hooks.version' => 'release-a']);
    $arguments = ['siteId' => '123', 'locale' => 'en', 'token' => 'x', 'empty' => ''];
    PostDeployHooks::dispatch('release-b', JobWithArguments::class, 1, $arguments);
    $this->work();
...
it('uses the configured expiry for command and direct construction while allowing overrides', function () {
    $this->freezeSecond();
    config(['post-deploy-hooks.job.expire' => 45]);
    ...
    expect((new PostDeployHooks('release-b', GenerateSitemap::class, 60))->expires)->toBe(60);
    ...
    config(['post-deploy-hooks.job.expire' => 5]);
...
it('fails unknown named arguments instead of silently dropping context', function () {
    PostDeployHooks::dispatch('release-b', GenerateSitemap::class, arguments: ['typo' => 'value']);
    $this->work();
